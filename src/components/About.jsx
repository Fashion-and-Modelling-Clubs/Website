      ...
      <div className="mx-auto max-w-page px-5 sm:px-8 lg:px-12">
        <Reveal>
          <div className="flex items-center gap-4">
            <span className="eyebrow text-ink/50">{about.label}</span>
            <span className="h-px w-20 rule-dark" aria-hidden="true" />
          </div>
        </Reveal>
        ...
        <Reveal delay={0.1}>
          <ul className="mt-14 flex flex-wrap items-baseline gap-x-5 gap-y-2 border-t border-ink/15 pt-7">
            {about.vocabulary.map((word, i) => (
              <li key={word} className="flex items-baseline gap-5">
                <span className="font-display text-2xl font-light sm:text-3xl">{word}</span>
          ...
          <div className="mt-16 overflow-hidden border-y border-ink/10 py-6">
            <div className="mb-5 flex items-center gap-4">
              <span className="eyebrow text-ink/50">PREVIOUS SPONSORS</span>
